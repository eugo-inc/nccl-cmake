...
def required_cuda(coll, redop, ty, algo, proto):
    ...
    if coll in ("SendRecv", "Generic", "Nop"):
        return (cudart, arch)

    if proto != "SIMPLE" and algo not in ("RING", "TREE"):
        return None
    ...
    if "NVLS" in algo:
        if coll in ("AllReduce", "Reduce", "ReduceScatter"):
            ...
            nvls_ok = (
                (ty in ("i32", "u32", "i64", "u64") and redop in ("Sum", "MinMax"))
...
if os.environ.get("NCCL_USE_CMAKE", "") == "1":
    print(files)
...
for name in name_to_funcs.keys():
    ...
    with open(os.path.join(gensrc, name), "w") as f:
        out = f.write
        out(f'#include "common.h"\n#include "{coll_camel_to_lower[coll]}.h"\n')
