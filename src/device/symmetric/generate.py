...
coll_to_lower = {"AllGather": "all_gather", "AllReduce": "all_reduce", "ReduceScatter": "reduce_scatter"}

red_to_ncclDevRedOp = {"sum": "ncclDevSum"}
red_to_Func = {"sum": "FuncSum"}
...
def kernel_conds(k):
    ...
    if not specific_sms:
        ...
        arch_cond = " || ".join(
            ["0"]
            + [
                "NCCL_CUDA_ARCH_%sSPECIFIC==%d"
                % ("FAMILY_" if sm[-1] == "f" else "", 10 * int(sm.replace("a", "").replace("f", "")))
                for sm in specific_sms
            ]
        )
...
def instantiate(k):
    ...
    if k.coll in reductions:
        inst = form_red_ty.format(
            cname=cname,
            id=id,
            red=red_to_Func[k.red],
            ty=ty_to_cxxtype[k.ty],
            cudart_cond=cudart_cond,
            arch_cond=arch_cond,
        )
    else:
