...
> Env vars can also be set in `/etc/nccl.conf` or `${NCCL_CONF_FILE}`.
...
| `NCCL_SOCKET_INLINE` | Socket inline threshold | — | — | `src/transport/net_socket.cc` | ⚙️ Internal |
| `NCCL_SOCKET_MIN_TASKSIZE` | Minimum task size for socket transport | — | — | `src/transport/net_socket.cc` | ⚙️ Internal |
...
| `NCCL_IB_PKEY` | InfiniBand Partition Key | — | — | `src/transport/net_ib/connect.cc` | ⚙️ Leave default unless fabric uses partitioning |
...
| `NCCL_IB_MERGE_VFS` | Merge VFs (Virtual Functions) in SR-IOV | — | — | `src/transport/net_ib/init.cc` | ⚙️ Internal, for VM/SR-IOV setups |
| `NCCL_IB_DATA_DIRECT` | Enable direct data path for IB | — | — | `src/transport/net_ib/init.cc` | ⚙️ Internal |
| `NCCL_IB_USE_INLINE` | Use inline data for small IB messages | — | — | `src/transport/net_ib/connect.cc` | ⚙️ Internal |
| `NCCL_IB_WARN_RAIL_LOCAL` | Warn about rail-local suboptimal NIC assignment | — | — | `src/transport/net_ib/connect.cc` | ⚙️ Internal debug |
| `NCCL_IB_ROUTABLE_FLID_GID_INDEX` | Routable FLID GID index | — | — | `src/transport/net_ib/connect.cc` | ⚙️ Internal |
...
| `NCCL_GDR_FLUSH_DISABLE` | Disable GDR flush | — | — | `src/transport/net_ib/connect.cc` | ⚙️ Internal |
| `NCCL_IB_MQP_RETRY_ALL` | Multi-QP retry for all operations | — | — | `src/misc/ibvwrap.cc` | ⚙️ Internal |
| `NCCL_IB_MQP_RETRY_CNT` | Multi-QP retry count | — | — | `src/misc/ibvwrap.cc` | ⚙️ Internal |
| `NCCL_IB_MQP_RETRY_SLEEP_MSEC` | Multi-QP retry sleep | — | — | `src/misc/ibvwrap.cc` | ⚙️ Internal |
...
| `NCCL_CONF_FILE` | Path to static configuration file (env vars as key=value lines) | `~/.nccl.conf` | 2.23 | `src/misc/param.cc` | 🔧 Set for system-wide config: `/etc/nccl.conf` |
...
| `NCCL_TOPO_DUMP_FILE_RANK` | Which rank dumps topology | — | — | `src/graph/topo.cc` | ⚙️ Set `0` to only dump from rank 0 |
...
| `NCCL_GRAPH_DUMP_FILE_RANK` | Which rank dumps graph | — | — | `src/init.cc` | ⚙️ Set `0` |
...
| `NCCL_NET_DISABLE_INTRA` | Disable intra-node net usage | — | — | `src/graph/paths.cc` | ⚙️ Internal |
...
| `NCCL_P2P_READ_ENABLE` | Enable P2P read mode | — | — | `src/transport/p2p.cc` | ⚙️ Internal |
| `NCCL_P2P_USE_CUDA_MEMCPY` | Use CUDA memcpy for P2P | — | — | `src/transport/p2p.cc` | ⚙️ Internal |
| `NCCL_SHM_DISABLE` | Disable shared memory transport | `0` | — | `src/transport/shm.cc` | 🚫 Only for debugging |
| `NCCL_SHM_LOCALITY` | SHM locality control | — | — | `src/transport/shm.cc` | ⚙️ Internal |
| `NCCL_SHM_MEMCPY_MODE` | SHM memory copy mode | — | — | `src/transport/shm.cc` | ⚙️ Internal |
| `NCCL_SHM_USE_CUDA_MEMCPY` | Use CUDA memcpy for SHM | — | — | `src/transport/shm.cc` | ⚙️ Internal |
...
| `NCCL_NET_OPTIONAL_RECV_COMPLETION` | Optional receive completion | — | — | `src/transport/net.cc` | ⚙️ Internal |
| `NCCL_NET_OVERHEAD` | Net overhead estimation for tuning | — | — | `src/graph/tuning.cc` | ⚙️ Internal |
| `NCCL_NET_FORCE_FLUSH` | Force net flush | — | — | `src/graph/paths.cc` | ⚙️ Internal |
| `NCCL_NET_PLUGIN_REF_COUNT` | Net plugin reference counting | — | — | `src/plugin/net.cc` | ⚙️ Internal |
...
| `NCCL_CONNECT_ROUND_MAX_PEERS` | Max peers per connection round | — | — | `src/transport.cc` | ⚙️ Internal |
| `NCCL_REPORT_CONNECT_PROGRESS` | Report connection progress during init | — | — | `src/transport.cc` | ⚙️ Internal, useful for debugging slow init |
| `NCCL_IGNORE_DISABLED_P2P` | Ignore disabled P2P settings | — | — | `src/graph/paths.cc` | ⚙️ Internal |
| `NCCL_P2P_PER_CHANNEL_NET_BW` | Per-channel net bandwidth for P2P paths | — | — | `src/graph/paths.cc` | ⚙️ Internal |
| `NCCL_MAX_P2P_NCHANNELS` | Max P2P channels | — | — | `src/graph/paths.cc` | ⚙️ Internal |
| `NCCL_MIN_P2P_NCHANNELS` | Min P2P channels | — | — | `src/graph/paths.cc` | ⚙️ Internal |
| `NCCL_MULTI_SEGMENT_REGISTER` | Multi-segment registration for generic transport | — | — | `src/transport/generic.cc` | ⚙️ Internal |
| `NCCL_SINGLE_PROC_MEM_REG_ENABLE` | Single-process memory registration | — | — | `src/group.cc` | ⚙️ Internal |
...
| `NCCL_P2P_EPOCH_ENABLE` | Enable P2P epoch mode | — | — | `src/enqueue.cc` | ⚙️ Internal |
...
| `NCCL_GDRCOPY_ENABLE` | Enable GDRCopy for small message optimization | — | — | `src/init.cc` | 🔧 **IB**: Enable if GDRCopy is installed |
| `NCCL_GDRCOPY_FIFO_ENABLE` | Enable GDRCopy for FIFO | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_GDRCOPY_FLUSH_ENABLE` | Enable GDRCopy flush | — | — | `src/transport/net.cc` | ⚙️ Internal |
| `NCCL_GDRCOPY_SYNC_ENABLE` | Enable GDRCopy sync | — | — | `src/transport/net.cc` | ⚙️ Internal |
...
| `NCCL_NVLS_NCHANNELS` | Number of NVLS channels | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_NVLS_CHUNKSIZE` | NVLS chunk size | — | — | `src/transport/nvls.cc` | ⚙️ Internal |
...
| `NCCL_MNNVL_RAIL_PER_HOST` | Rails per host for MNNVL | — | — | `src/graph/search.cc` | ⚙️ Internal |
| `NCCL_MNNVL_SCATTER_NETS_ENABLE` | Scatter nets for MNNVL | — | — | `src/graph/search.cc` | ⚙️ Internal |
...
| `NCCL_LL_BUFFSIZE` | Low-latency buffer size | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_LL128_BUFFSIZE` | LL128 buffer size | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_LL128_C2C` | LL128 for C2C paths | — | — | `src/graph/tuning.cc` | ⚙️ Internal |
| `NCCL_LL128_NTHREADS` | Thread count for LL128 | — | — | `src/graph/tuning.cc` | ⚙️ Internal |
...
| `NCCL_MAX_NCHANNELS` | Max communication channels. Superseded by `NCCL_MAX_CTAS`. | — | 2.5.0 | `src/graph/connect.cc` | 🚫 Use `NCCL_MAX_CTAS` instead |
...
| `NCCL_MAX_NRINGS` | Legacy alias for `MAX_NCHANNELS` | — | 2.0.5 | `src/graph/connect.cc` | 🚫 Deprecated |
| `NCCL_MIN_NRINGS` | Legacy alias for `MIN_NCHANNELS` | — | 2.2.0 | `src/graph/connect.cc` | 🚫 Deprecated |
...
| `NCCL_UNPACK_DOUBLE_NCHANNELS` | Double channels for unpacking | — | — | `src/graph/connect.cc` | ⚙️ Internal |
| `NCCL_NCHANNELS_PER_NET_PEER` | Channels per network peer | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_P2P_NET_CHUNKSIZE` | Network chunk size for ncclSend/ncclRecv | `131072` (128K) | 2.14 | `src/init.cc` | ⚙️ Power of 2, next up: 262144 |
| `NCCL_P2P_NVL_CHUNKSIZE` | NVLink chunk size for P2P | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_P2P_PCI_CHUNKSIZE` | PCI chunk size for P2P | — | — | `src/init.cc` | ⚙️ Internal |
...
| `NCCL_NVLSTREE_MAX_CHUNKSIZE` | Max chunk size for NVLSTree | — | — | `src/enqueue.cc` | ⚙️ Internal |
| `NCCL_PAT_ENABLE` | Enable PAT algorithm | — | — | `src/graph/tuning.cc` | ⚙️ Internal |
| `NCCL_P2P_SCHEDULE_GROUP_SIZE` | P2P schedule group size | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_CHUNK_SIZE` | Generic chunk size | — | — | `src/enqueue.cc` | ⚙️ Internal |
| `NCCL_SYM_CE_THRESHOLD` | Symmetric CE threshold | — | — | `src/enqueue.cc` | ⚙️ Internal |
| `NCCL_ALLGATHERV_ENABLE` | Enable AllGatherV | — | — | `src/enqueue.cc` | ⚙️ Internal |
...
| `NCCL_WIN_STRIDE` | Window stride | — | — | `src/dev_runtime.cc` | ⚙️ Internal |
...
| `NCCL_SET_CPU_STACK_SIZE` | Set CPU stack size (for NCCL's CPU threads) | — | — | `src/init.cc` | ⚙️ Internal |
...
| `NCCL_L1_SHARED_MEMORY_CARVEOUT` | L1/shared memory carveout for NCCL kernels | — | — | `src/enqueue.cc` | ⚙️ Internal |
| `NCCL_GROUP_CUDA_STREAM` | Group CUDA stream setting | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_GRAPH_HELPER_DISABLE` | Disable graph helper | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_WORK_FIFO_BYTES` | Work FIFO size | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_WORK_ARGS_BYTES` | Work args size | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_ENABLE_VERSION_CHECK` | Enable device runtime version check | — | — | `src/dev_runtime.cc` | ⚙️ Internal |
| `NCCL_CREATE_THREAD_CONTEXT` | Create thread context | — | — | `src/proxy.cc` | ⚙️ Internal |
| `NCCL_PROXY_CPUSET` | Pin proxy thread to specific CPUs (comma-separated ranges) | — | — | `src/proxy.cc` | 🔧 Set for NUMA-aware proxy pinning |
| `NCCL_PROXY_APPEND_BATCH_SIZE` | Proxy append batch size | — | — | `src/proxy.cc` | ⚙️ Internal |
| `NCCL_PROXY_DUMP_SIGNAL` | Dump proxy state on signal | — | — | `src/proxy.cc` | ⚙️ Internal debug |
| `NCCL_PROGRESS_APPENDOP_FREQ` | Progress appendop frequency | — | — | `src/proxy.cc` | ⚙️ Internal |
| `NCCL_NVTX_DISABLE` | Disable NVTX annotations | — | — | `src/init_nvtx.cc` | ⚙️ Set `1` if NVTX overhead is unwanted |
| `NCCL_NVB_PRECONNECT` | Preconnect NVB peers | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_NVLINK_UTIL_CENTRIC_SCHED_ENABLE` | NVLink utilization-centric scheduling | — | — | `src/init.cc` | ⚙️ Internal |
...
| Env Var | Description | Default | Since | Source | Recommendation |
|---------|-------------|---------|-------|--------|----------------|
| `NCCL_GIN_ENABLE` | Enable GPU-Initiated Networking | — | 2.29 | `src/gin/gin_host.cc` | ⚙️ Experimental. Requires DOCA GPUNetIO hardware |
| `NCCL_GIN_TYPE` | GIN backend type | — | 2.29 | `src/gin/gin_host.cc` | ⚙️ Internal |
| `NCCL_GIN_NCONTEXTS` | Number of GIN contexts | — | 2.29 | `src/gin/gin_host.cc` | ⚙️ Internal |
| `NCCL_GIN_COUNTER_POOL_SIZE` | GIN counter pool size | — | 2.29 | `src/gin/gin_host.cc` | ⚙️ Internal |
| `NCCL_GIN_SIGNAL_POOL_SIZE` | GIN signal pool size | — | 2.29 | `src/gin/gin_host.cc` | ⚙️ Internal |
| `NCCL_GIN_PROXY_QUEUE_SIZE` | GIN proxy queue size | — | 2.29 | `src/gin/gin_host_proxy.cc` | ⚙️ Internal |
| `NCCL_GIN_GDAKI_NIC_HANDLER` | GDAKI NIC handler config | — | 2.29 | `src/transport/net_ib/gdaki/gin_host_gdaki.cc` | ⚙️ Internal |
| `NCCL_GIN_GDAKI_QP_DEPTH` | GDAKI queue pair depth | — | 2.29 | `src/transport/net_ib/gdaki/gin_host_gdaki.cc` | ⚙️ Internal |
| `NCCL_GIN_ERROR_QUERY_SEC` | GIN error query interval (seconds) | — | 2.29 | `src/transport/net_ib/gdaki/gin_host_gdaki.cc` | ⚙️ Internal |
...
| `NCCL_SYM_CTAS` | Symmetric CTAs | — | — | `src/sym_kernels.cc` | ⚙️ Internal |
| `NCCL_SYM_NOWIN_ENABLE` | Symmetric scheduling without windows | — | — | `src/scheduler/symmetric_sched.cc` | ⚙️ Internal |
| `NCCL_COMM_SPLIT_SHARE_RESOURCES` | Share resources on comm split | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_COMM_SHRINK_SHARE_RESOURCES` | Share resources on comm shrink | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_NUM_RMA_CTX` | Number of RMA contexts | — | — | `src/init.cc` | ⚙️ Internal |
| `NCCL_RMA_PROXY_QUEUE_SIZE` | RMA proxy queue size | — | — | `src/rma/rma_proxy.cc` | ⚙️ Internal |
| `NCCL_RMA_PROXY_DUMP_SIGNAL` | Dump RMA proxy state on signal | — | — | `src/rma/rma_proxy.cc` | ⚙️ Internal debug |
